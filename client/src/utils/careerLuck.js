import { stemDetailedInterpretation, elementProperties, ageCharacteristics } from "../constants/mappings";
...
export function calculateCareerLuck(pillars, options = {}) {
    try {
        ...
        // 기본 정보 추출
        const yearStem = pillars?.year?.stem?.value || '';
        const yearBranch = pillars?.year?.branch?.value || '';
        ...
        let prevYearStem = null; // 전년도 천간 저장 변수
...
/**
 * 전년도 천간과 올해 천간의 전환 관계에 따른 해석 반환 함수
 */
function analyzeTransition(lastYearStem, currentYearStem, year) {
    // 전환 관계에 따른 해석 매핑 (예시)
    const transitionMapping = {
        "甲-乙": `${year}년은 작년 甲의 강인한 기운에서 벗어나, 을의 부드러운 조화와 협력이 돋보이는 해입니다. 새로운 관계 형성과 변화의 기회가 많아, 기존 방식에서 벗어난 유연한 대처가 필요합니다.`,
        "乙-丙": `${year}년은 작년의 유연함이 새로운 열정과 도전으로 이어지는 시기입니다. 적극적인 자세로 새로운 기회를 창출해야 합니다.`,
    ...
    return transitionMapping[key] || null;
}
...
function getAgeBasedAdvice(ageCharacteristic, stem) {
    const advices = {
        ...
        career_growth: {
            "甲": "중간 관리자로서의 역량을 키우고, 팀 운영 스킬을 향상시키는데 집중하세요.",
            "乙": "전문성을 깊이 있게 발전시키고, 업계 네트워크를 확장하는데 주력하세요.",
            // ... 다른 천간들에 대한 조언
        },
        // ... 다른 연령대별 조언
    };
...
function analyzeDayRelation(yearStem, dayStem) {
    // 일간과 년간의 관계에 따른 상세 분석
    const relationKey = `${dayStem}-${yearStem}`;
    const relations = {
        "甲-乙": "올해는 자신의 고유한 업무 스타일을 발전시키기 좋은 시기입니다. 새로운 아이디어를 적극적으로 도입하고, 자신의 능력을 향상시키는 데 집중하세요.",
        "乙-丙": "창의력과 실행력이 조화를 이루는 시기로, 새로운 프로젝트를 시작하기에 적합합니다. 자신의 아이디어를 현실화하고, 팀과 협력하여 성과를 높일 수 있습니다.",
        "丙-丁": "열정과 추진력이 강조되는 해입니다. 자신의 목표를 명확히 하고, 그에 따라 적극적으로 행동하면 좋은 결과를 얻을 수 있습니다.",
        "丁-戊": "안정성과 신뢰성이 중요한 시기입니다. 꾸준한 노력과 성실함으로 주변의 신뢰를 얻고, 장기적인 목표를 설정하여 추진하는 것이 좋습니다.",
        "戊-己": "세부적인 계획 수립과 내실 다지기에 적합한 해입니다. 자신의 역량을 강화하고, 전문성을 높이는 데 주력하세요.",
        "己-庚": "변화와 혁신의 기운이 강한 시기입니다. 새로운 도전과 변화를 두려워하지 말고, 적극적으로 수용하여 발전의 기회로 삼으세요.",
        "庚-辛": "섬세함과 철저함이 요구되는 해입니다. 세부적인 부분까지 신경 쓰며, 완성도를 높이는 데 집중하면 성과를 거둘 수 있습니다.",
        "辛-壬": "유연성과 적응력이 중요한 시기입니다. 다양한 상황에 능동적으로 대처하고, 새로운 환경에 빠르게 적응하는 능력을 키우세요.",
        "壬-癸": "지혜와 통찰력이 돋보이는 해입니다. 깊이 있는 사고와 분석을 통해 문제를 해결하고, 장기적인 관점에서 계획을 수립하는 것이 좋습니다.",
        "癸-甲": "새로운 시작과 도전의 기운이 강한 시기입니다. 과거의 경험을 바탕으로 새로운 분야에 도전하고, 자신의 영역을 확장해보세요."
    };
    return relations[relationKey] || "기본적인 업무 역량을 강화하는데 집중하세요.";
}
